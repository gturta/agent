...
use rmcp::{RoleClient, transport::{StreamableHttpClientTransport, child_process::TokioChildProcess}, service::{ServiceExt, RunningService}};
...
impl McpTools {
    pub async fn build(config: McpConfig) -> Result<Self> {
        ...
        for mcp_tool in mcp_tools {
            let tool = McpTool::from_mcp_tool(mcp_tool);
...
impl McpTool {
    pub fn from_mcp_tool(tool: rmcp::model::Tool) -> Self {
        McpTool{
